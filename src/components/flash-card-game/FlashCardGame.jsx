...
const FlashCardGame = () => {
  ...
  return (
     <div>
      ...
      <div>
      {
        model.answerOptions.map((it, i) => <FlashCardAnswer onClick={() => checkAnswer(it)} key={i} character={it}></FlashCardAnswer>)
      }
      </div>
      ...
      (
        <div>
          ...
          <div>
            {model.questionPool.map((it, i) => <span key={i}>{it.hangul}</span>)}
          </div>

          Correct answers ({model.correctAnswers.length})
          <div>
            {model.correctAnswers.map((it, i) => <span key={i}>{it.hangul}</span>)}
          </div>

          Incorrect answers ({model.incorrectAnswers.length})
          <div>
            {model.incorrectAnswers.map((it, i) => <span key={i}>{it.hangul}</span>)}
          </div>
        </div>
